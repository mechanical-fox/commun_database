...
Ce projet à pour but de permettre de lancer une base de donnée PostgreSQL, depuis un serveur     
...
indiquant celui-ci lors du 1er démarrage de la base de donnée.    
...
base de donnée, ce qui veut dire que une fois la première execution faite, il vous sera impossible    
...
2. Créez un répertoire "data" qui sera utilisé par la commande docker. Concrétement, l'on va     
demander à docker de stocker tout le contenu de la base de donnée dans ce répertoire, et non dans    
un volume, afin de pouvoir facilement copier la base de donnée dans le futur.    
...
base de donnée dans le dossier "data" crée à l'étape précédente. Ceci est la commande lors du premier     
démarrage, ou vous devrez indiquer le mot de passe à choisir pour la base de donnée. Pensez à modifier    
